...
export function AnalysisDashboard({ analysis }: AnalysisDashboardProps) {
  ...
  const getVulnerabilityColor = (score: number) => {
    if (score >= 8) return 'bg-red-900/50 border-red-400/50 text-red-300';
    if (score >= 5) return 'bg-orange-900/50 border-orange-400/50 text-orange-300';
    return 'bg-yellow-900/50 border-yellow-400/50 text-yellow-300';
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6 animate-fade-in">
      {/* Advocate's Brief */}
      <Card className="bg-card/60 backdrop-blur-sm shadow-xl shadow-black/20">
        <CardHeader>
          <div className="flex items-center gap-3">
            <div className="p-2 bg-sky-900/50 rounded-lg"><Scale className="h-6 w-6 text-sky-300" /></div>
            <CardTitle className="font-headline text-2xl text-sky-300">Advocate's Brief</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <Accordion type="single" collapsible className="w-full">
            {analysis.advocateBrief.map((item, index) => (
              <AccordionItem value={`item-${index}`} key={index} className="border-slate-700/50">
                <AccordionTrigger className="hover:no-underline text-left text-slate-200">{item.argument}</AccordionTrigger>
                <AccordionContent>
                  <p className="font-semibold text-sm mb-2 text-slate-400">Case Citations & Relevance:</p>
                  <div className="space-y-4">
                    {item.caseCitations.length > 0 ? (
                       item.caseCitations.map((citation, i) => (
                          <div key={i} className="pl-4 border-l-2 border-blue-400/30">
                            <Button
                              variant="link"
      ...
      <Card className="bg-card/60 backdrop-blur-sm shadow-xl shadow-black/20">
        <CardHeader>
           <div className="flex items-center gap-3">
            ...
            <CardTitle className="font-headline text-2xl text-red-300">Identified Weaknesses</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {analysis.identifiedWeaknesses.map((weakness, index) => (
                <div key={index} className="p-4 bg-background/50 rounded-lg border border-red-500/20">
                    <p className="font-semibold text-slate-200">{weakness.weakness}</p>
      ...
      <Card className="bg-card/60 backdrop-blur-sm shadow-xl shadow-black/20">
        ...
        <CardContent className="space-y-6">
          <div>
           ...
           <div>
            <h4 className="font-semibold mb-2 flex items-center gap-2 text-amber-300"><Lightbulb className="h-5 w-5"/>Refined Strategy</h4>
            <ul className="list-disc list-inside space-y-1 text-sm text-slate-300">
      ...
      <Card className="bg-card/60 backdrop-blur-sm shadow-xl shadow-black/20 lg:col-span-2 xl:col-span-1">
        <CardHeader>
            <div className="flex items-center gap-3">
            ...
            <div className="space-y-4">
                ...
                <Accordion type="multiple" className="w-full">
                    {analysis.adversarialPlaybook.potentialCounterArguments.map((item, index) => (
                      <AccordionItem value={`counter-${index}`} key={index} className="border-slate-700/50">
                          <AccordionTrigger className="hover:no-underline text-left text-slate-200">{item.counterArgument}</AccordionTrigger>
                          <AccordionContent>
                            <div className="pl-4 border-l-2 border-purple-400/30 space-y-4">
                              {item.rebuttals.map((rebuttal, rIndex) => (
                                <div key={rIndex}>
                                  <p className="font-semibold text-sm text-slate-400 flex items-center"><MessageCircle className="h-4 w-4 mr-2" /> Our Rebuttal</p>
                                  <p className="text-sm mt-1 text-slate-300">{rebuttal.rebuttal}</p>

                                  {rebuttal.potentialCounterRebuttals.length > 0 && (
                                    <div className="mt-3 pl-4 border-l-2 border-slate-700/70">
                                      ...
                                      <ul className="space-y-2 text-sm">
                                        {rebuttal.potentialCounterRebuttals.map((counter, cIndex) => (
                                          <li key={cIndex} className="flex items-start gap-2">
                                            <ChevronRight className="h-4 w-4 mt-0.5 shrink-0 text-slate-600" />
                                            <div>
                                  ...
                                  {rebuttal.citations.length > 0 && (
                                    <div className="mt-3">
                                      <p className="font-semibold text-xs text-slate-500 mb-1">Citations:</p>
